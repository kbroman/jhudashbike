...
accident.icon = readPNG("chart.png")
...
for(i in 1:length(bike.paths)){
	m <- m %>% addPolylines(lng=as.matrix(bike.paths[[i]])[,1], lat=as.matrix(bike.paths[[i]])[,2]) %>% addMarkers(lng=as.matrix(bike.paths[[i]])[,1], lat=as.matrix(bike.paths[[i]])[,2])
    print(i) 	
}



m <- m %>% addMarkers(lng=as.matrix(bike.paths[[i]])[,1], lat=as.
