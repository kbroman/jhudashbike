...
x <- cbind(as.numeric(as.character(accident.cords$latitude)),  as.numeric(as.character(accident.cords$longitude)))


index.polygon <- point.in.polygon(x[,2], x[,1], pol.x = bdy[,1], pol.y = bdy[,2], mode.checked=FALSE)

accident.cords <- accident.cords[index.polygon == 1,]  

write.dir <- '/Users/elizabethsweeney/Dropbox/Elizabeth_Sweeney_Documents/Current_Projects/jhudashbike/clean_data/'

save(accident.cords, crashIcon, file = paste0(write.dir , 'accidents.rda'))
save(bike.paths, file = paste0(write.dir , 'bikepaths.rda'))
